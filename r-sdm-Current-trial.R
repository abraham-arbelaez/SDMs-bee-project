# New trial with method from STAT764 (Activity3)
...
df.BIMP.final <- read.csv("df.BIMP.final.csv")
df.BGRIS.final <- read.csv("df.BGRIS.final.csv")
df.BFRAT.final <- read.csv("df.BGRIS.final.csv")


# Fit IPP following EQ example (GLMs)
#m1 <- glm(y ~ dist.to.well,
          #family=poisson(link="log"),
          #weights=100^(1-df.ipp$y),
          #data=df.ipp)
#summary(m1)
#confint.default(m1)
...
# Fit IPP following EQ example (GAMs)
...
bpen.coef.grass <- coef(m2.bpen, parm = "grass.perc")[2]
bpen.uCI.grass <- confint.default(m2.bpen, parm = "grass.perc")[2]
bpen.lCI.grass <- confint.default(m2.bpen, parm = "grass.perc")[1]

BPEN.coef.grass <- coef(m2.BPEN, parm = "grass.perc")[2]
BPEN.uCI.grass <- confint.default(m2.BPEN, parm = "grass.perc")[2]
BPEN.lCI.grass <- confint.default(m2.BPEN, parm = "grass.perc")[1]

bimp.coef.grass <- coef(m2.bimp, parm = "grass.perc")[2]
bimp.uCI.grass <- confint.default(m2.bimp, parm = "grass.perc")[2]
bimp.lCI.grass <- confint.default(m2.bimp, parm = "grass.perc")[1]
...
par(mar=c(4,7,1,1))
plotCI(1, bpen.coef.grass, ui=bpen.uCI.grass, li=bpen.lCI.grass,pch=20,xaxt="n",xlab="",ylab="Estimated regression coefficient \n (% grass within 2km)")
lines(1, c(0,0),col="gold",lwd=3)

par(mar=c(4,7,1,1))
plotCI(c(1:3), beta.1.hat, ui=ucl, li=lcl,pch=20,xaxt="n",xlab="",ylab="Estimated regression coefficient \n (% grass within 5km)")
lines(c(1,3),c(0,0),col="gold",lwd=3)
axis(at=c(1:3),lab=c("Pois","NB","ZIP"),side=1)
...
summary.table <- data.frame(bpen.grass = mean(pres.bpen$grass.perc),
                 BPEN.grass = mean(pres.BPEN$grass.perc),
                 rand.grass = mean(rand$grass.perc),
                 bpen.devLow = mean(pres.bpen$devLow.perc),
                 BPEN.devLow = mean(pres.BPEN$devLow.perc),
                 rand.devLow = mean(rand$devLow.perc),
                 bpen.devHigh = mean(pres.bpen$devHigh.perc),
                 BPEN.devHigh = mean(pres.BPEN$devHigh.perc),
                 rand.devHigh = mean(rand$devHigh.perc))
summary.table



# Scale and plot intensity function ????????
lambda.bpen <- (10000/10157)*((sum(df.bpen.final$pa)/sum(exp(predict(m2.bpen,df.bpen.final[which(df.bpen.final$pa==0),]))))*exp(predict(m2.bpen,df.bpen.final[which(df.bpen.final$pa==0),])))
plot(df.bpen.final[which(df.bpen.final$pa==0),]$grass.perc,lambda.bpen,
     ylim=c(0,1),
     xlab="grassland percentage within 2 km",ylab="Expected presence of BPEN")
...
# Fit models to bumble bee data (presence-only for IPPP)
...
#####################################################################################
#
# PREDICTION MAPPING (IPP models) --  W/ spatstat
#
####################################################################################
...
# Filter to presence-only records
...
ppp.pred.bimp.p <- as.ppp(sf.pred.bimp.p)
marks(ppp.pred.bimp.p) <- sf.pred.bimp.p
...
### Following EQ example
lambda <- (10000/213096)*((sum(df.bpen.final$pa)/sum(exp(predict(
  m2.bpen,df.bpen.final[which(df.bpen.final$pa == 0),]))))*exp(predict(
    m2.bpen, df.bpen.final[which(df.bpen.final$pa == 0),])))

plot(lambda)

### Following Dovers et al. example
# Create image from coordinates and predicted values

pred.im <- as.im(quad[, c("x", "y", "z")],
                 W = gorillas$window)

pred.im <- as.im(ppp.pred.bpen[,c("X", "Y", "pred")],
                 W = sf.ks.boundary)
...
## Display density plots side-by-side (suggested by CoPilot)
install.packages("patchwork")
library(patchwork)

#Stacked boxplots of land cover distribution within 2km buffer
bp.bpen + bp.BPEN + bp.bimp + bp.BIMP

#Joined summary table for bpen & bimp
...
#####################################################################################
#
# PREDICTION MAPPING (Alternative models) --  W/ spatstat
#
####################################################################################
#convert df to sf
